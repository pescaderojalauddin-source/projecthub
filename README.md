...
### Дополнительный функционал (quick-wins v1–v4)

Сверх обязательных требований ТЗ в проект встроено 20+ небольших, но видимых
улучшений: личный дашборд «Мой день» с графиками Chart.js, глобальный поиск в
навбаре, прогресс-бары проектов, аватарки-инициалы, избранные проекты со
звёздочкой, конфетти при закрытии задачи, календарь дедлайнов, хоткеи
(`/`, `?`, `g d`, `g p`, `n`, `t`), toast-уведомления, эмодзи-иконки проектов,
Markdown в описаниях, ачивки, приветствие по времени дня, экспорт задач в CSV,
приоритет задач, теги, burndown-график, @-меншены в комментариях, вложения
файлов, утренний email-дайджест о просроченных задачах. Полный список и где
это смотреть — [`WHATS-DONE.md`](WHATS-DONE.md).
...
- [`README.md`](README.md) — этот файл (быстрый старт, стек, доступы).
- [`WHATS-DONE.md`](WHATS-DONE.md) — полный список реализованного функционала с указанием файлов.
- [`HOW-IT-WORKS.md`](HOW-IT-WORKS.md) — экскурсия по архитектуре: слои, пакеты, как работает каждая фича, рецепт добавления новой.
- [`TZ-COMPLIANCE.md`](TZ-COMPLIANCE.md) — таблица соответствия каждого пункта ТЗ его реализации.
- [`DEPLOYMENT.md`](DEPLOYMENT.md) — пошаговая инструкция по деплою (Render, Railway, Fly.io, VPS) с переменными окружения.
